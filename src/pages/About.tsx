  ...
  return (
    <div style={{ perspective: 700, flexShrink: 0 }}>
      <motion.div
        ...
        style={{
          width: size, height: size, borderRadius: 14, position: 'relative',
    ...
    <div style={{ minHeight: '100vh', paddingTop: 80, position: 'relative' }}>
      <div style={{ maxWidth: 900, margin: '0 auto', padding: isMobile ? '1rem 1.2rem 4rem' : '2rem 2rem 6rem' }}>
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <h2 style={{ fontFamily: 'Playfair Display, serif', fontSize: isMobile ? '1.9rem' : '2.5rem', fontWeight: 900, marginBottom: '0.5rem', color: 'white' }}>From Village to Wall Street</h2>
          <div style={{ width: 48, height: 3, background: '#c9a84c', borderRadius: 2, marginBottom: '1.5rem' }} />
          <p style={{ color: 'rgba(255,255,255,0.6)', fontSize: '1rem', lineHeight: 1.8, marginBottom: isMobile ? '2rem' : '3rem', maxWidth: 600 }}>
            Every step of this journey shaped who I am. From a small village in Andhra Pradesh to Bank of America in New York.
          </p>
        </motion.div>

        <div style={{ position: 'relative' }}>
          {!isMobile && <div style={{ position: 'absolute', left: 24, top: 0, bottom: 0, width: 2, background: 'rgba(201,168,76,0.2)' }} />}

          {journey.map((item, i) => (
            <motion.div key={i}
              initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.08 }}
              style={{ display: 'flex', gap: isMobile ? '0.8rem' : '2rem', marginBottom: '2.2rem', position: 'relative' }}>
              ...
              <div style={{
                ...
                borderRadius: 12,
                padding: isMobile ? '1.1rem' : '1.2rem 1.4rem',
                ...
                <div style={{ width: '100%' }}>
                  <div style={{ color: '#c9a84c', fontSize: '0.78rem', letterSpacing: 2, textTransform: 'uppercase', marginBottom: '0.3rem' }}>
                    {isMobile ? `${item.emoji}  ${item.year}` : item.year}
                  </div>
                ...
                <Photo3D img={item.img} title={item.title} index={i} isMobile={isMobile} />
              </div>
            </motion.div>
          ))}
        </div>
